...
@if(!empty($tracking_numbers))
    <div style="border-bottom: 1px solid #bfbfbf; font-size: 15px; margin-bottom: 10px; padding-bottom: 6px;">
        <p style="margin-bottom: 5px;">Below is information on tracking the shipment of this order.</p>
        @if(count($tracking_numbers) == 1)
            <h4 style="margin: 0;">Carrier: {{ $tracking_numbers[0]['key'] }}</h4>
            <h4 style="margin: 0 0 10px 0;">Tracking Number: <a
                        href="{{ tracking_link($tracking_numbers[0]['key'], $tracking_numbers[0]['value']) }}">{{ $tracking_numbers[0]['value'] }}</a>
            </h4>
        @else
            <table>
                <tbody>
                ...
                @foreach($tracking_numbers as $tn)
                    <tr>
                        <td>Package {{ $package_num }}:</td>
                        <td><strong>Carrier</strong>: {{ $tn['key'] }}</td>
                        <td><a href="{{ tracking_link($tn['key'], $tn['value']) }}">{{ $tn['value'] }}</a></td>
                    </tr>
                    {{ $package_num++ }}
                @endforeach
                </tbody>
